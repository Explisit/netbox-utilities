...
.VERSION_CONTROL
	...
	Author:			Svet
	Creation Date:  2018-02-08
	Purpose/Change: Adding OS Name if vmware tools are present, change logic of netbox object creation, add vmware tools version and status, add datastore and HW version 
        ...
        foreach ($Cluster in $Clusters) {
            ...
            foreach ($VM in $VMs) {
                ...
                if ($Response.Count -eq 0) {
                    ...
                    $vCenterInfo["custom_fields"] = @{"vcenter_persistent_id" = $VM.PersistentID}
                ...
                if ($Guest.Hostname) {
                    ...
                    # Convert Guest OS name to Netbox vm_guest_name
					$vCenterInfo["custom_fields"] += @{"vm_guest_name" = $Hostname}
                } else {
                # Set vm_guest_name to default string
			    $vCenterInfo["custom_fields"] += @{"vm_guest_name" = "NO Vmtools OS Name Given"}
					}
				
				# Get vmware tools version				
				if ($Guest.ToolsVersion) {
				  $vCenterInfo["custom_fields"] += @{"vmtools_version" = $Guest.ToolsVersion.ToLower()}
				}
                else {
                    # Use VM inventory name as a placeholder - uniquely identified by PersistentID
                 $vCenterInfo["custom_fields"] += @{"vmtools_version" = "No VMware tools Version Available"}
				 }
				
				# Get vmware tools Status
				$vCenterInfo["custom_fields"] += @{"vmtools_status" = $VM.Guest.ExtensionData.ToolsStatus.ToString()}
				
				# Get VM HW Version
				$vCenterInfo["custom_fields"] += @{"vm_hw_version" = $VM.Version.ToString()}
				
				# Get VM Datastores
				$vm_datastores = $VM | Get-Datastore | Select Name
				$vCenterInfo["custom_fields"] += @{"vm_datastores" = $vm_datastores.Name}
				
				# Get Adapter VMware Network Name
				$vm_networks = $VM | Get-NetworkAdapter
				$vm_networks = $vm_networks | Select Name,Type,NetworkName
				$vCenterInfo["custom_fields"] += @{"vm_network_info" = $vm_networks}
...
$LogPath = "E:\GRAMMA\Powercli\Logs" + $Date + "_vcenter_netbox_sync.log"
